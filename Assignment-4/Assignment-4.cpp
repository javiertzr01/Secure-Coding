...
bool SSE::handleCall(const CallCFGEdge* calledge){
    ICFGNode* callNode = calledge->getSrcNode();
    ICFGNode* FunEntryNode = calledge->getDstNode();
    // callstack.push_back(calledge->getCallSite());
    getSolver().push();
    for(auto callPE : calledge->getCallPEs()){
...
bool SSE::handleRet(const RetCFGEdge* retEdge){
    // ICFGNode* retNode = retEdge->getDstNode();
    expr rhs = getCtx();
    if(const RetPE* retPE = retEdge->getRetPE()){
        expr rhs = getEvalExpr(getZ3Expr(retPE->getRHSVarID()));
    }
    getSolver().pop();
    if(const RetPE* retPE = retEdge->getRetPE()){
    ...
    // if(callstack.size != 0){
    //     if(callstack.back() == getCallICFGNode(retNode)){
    //         callstack.pop_back();
    //         getSolver().pop();
    //     }
    //     else{
...
bool SSE::handleBranch(const IntraCFGEdge* edge){
    ...
    const SVFValue* SVFVal = edge->getCondition();
    expr cond = getZ3Expr(svfir->getValueNode(SVFVal));
    expr successorVal = getZ3Expr(edge->getSuccessorCondValue());
...
bool SSE::handleNonBranch(const IntraCFGEdge* edge){
    ...
    for (const SVFStmt *stmt : dstNode->getSVFStmts())
    {
        ...
        else if (const GepStmt *gep = SVFUtil::dyn_cast<GepStmt>(stmt))
        {
            ...
            u32_t offset = z3Mgr->getGepOffset(gep);
