...
{
    string path_string = "START: ";
    for (auto edge: path){
        path_string = path_string + to_string(edge->getDst()->getNodeID()) + "->";
    }
    path_string += "END";
...
{
    visited.insert(src_edge->getDst());
    path.push_back(src_edge);
    if (src_edge->getDst() == dst){
        printPath(path);
    }
    for (auto edge : src_edge->getDst()->getOutEdges()){
        const bool isVisited = visited.find(edge->getDst()) != visited.end();
        if (!isVisited){
            DFS(edge, dst);
        }
    }
